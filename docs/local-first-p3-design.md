...
**P3c — 溶解 ServerMode(UI 主刀)** ✅ **P3c-1 完成(8a81c70 + 修复入 80bd437);P3c-2 完成(2e32f8d)**
...
- **P3c-2 交付(2e32f8d)**:ServerMode/ServerConfig 类型全删(纯函数 `resolveLegacyCloudSetup` 承接一次性 prefs 迁移,测试 8 例改写);`_cloudOrigin` 单字段(恒有值默认 Mica Cloud);凭证 per-origin 键(`authToken:<origin>`),换服务器保凭证回切免重登(`_signOut` 拆「清凭证 + _disconnectCloudSession」);Settings「Server」节 radio tiles 删除,只剩「云服务器」URL 节。
