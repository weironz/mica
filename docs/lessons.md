...
### 同一个配置名被两条代码路径各读各的 —— 修了 token,没修 server

2026-08-12 修过一次:`mica-cli mcp` 读 `MICA_PAT`,其余子命令只读 `MICA_TOKEN`。
照 MCP 文档配好、再跑 `mica-cli ws list`,得到「not logged in」,而报错还不肯说它想要
哪个名字。修法是把顺序抽成 `pick_token` 一处定义,并让退休的名字给出**点名替代者**的
报错。

**2026-08-28 用户报:`MICA_API_BASE_URL` 在 0.13.36 上不生效,报 `no server`。**
第一反应是"文档滞后了"。实测三种组合之后是另一回事:

| 组合 | 结果 |
| --- | --- |
| `MICA_API_BASE_URL` + `ws list` | `no server` |
| `MICA_SERVER` + `ws list` | 过(401,假 token) |
| `MICA_API_BASE_URL` + `mcp` | 过 |

**文档没滞后** —— `docs/mcp-connect.md` 写的是 `mcp` 子命令,而代码在那条路上跟文档
一字不差。真正的缺陷是 `MICA_API_BASE_URL` **只有 `mcp` 认**:同一个坑,差一个字段,
而且是在前一次修复之后**又活了两周多**。

两条教训:

1. **"修了 A,B 是同构的"这句话,当时没人说出来。** `pick_token` 的注释详尽记录了
   token 那半边的故事,却没有人问"还有哪个配置项是这个形状"。同构的缺陷不会因为你修好
   了其中一个就自己消失 —— 修的时候就要去数还有几个。
2. **用户的诊断可以是对的结论 + 错的归因,两者都要单独验。** "不生效"是对的(对绝大多数
   命令而言),"文档滞后"是错的。直接按归因去改文档,会把一份**正确**的文档改坏,而真正
   的 bug 原样留着。

现在 `pick_server` / `no_server_hint` 与 `pick_token` / `no_token_hint` 一一对应,
两个都有钉住顺序的测试,报错也都点名全部可接受的拼法。
