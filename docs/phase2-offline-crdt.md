...
| **P2-M4 云同步** 🟡 | ~~流 + 续传 + SV 兜底~~ ✅ 主干已上线(`sync.rs` push/fold/catch_up/diff_from_base + `ws.rs` sync.* + `sync_pg.rs` 真 PG 测试;2026-07-29 核实,此行长期漏标)。**残留**:op 模型退役 + 存量清理(见 roadmap「op 模型表无界增长」) | 主干 ✅,退役未做 |
