...
| **P2-M3 编辑器绑定** 🟡 | 自绘编辑器的单一 op 出口(`controller._send`→`onOps`)在本地离线模式接 `LocalDocBackend`(`lib/local/local_doc.dart`):每个 `DocOp` 镜像进 `MicaDocument`(yrs),快照防抖落 `MicaStore`(SQLite);开档时从 store 读回 blocks 给编辑器。**core**:粗粒度 `MicaDocument.update_block(kind?/text?/data?)` 对齐编辑器 update_block(有 text→text+marks 一起;仅 data→`set_block_marks` 调和 marks,无 marks 即清,对应 turn-into 重置语义);`marks_from_runs` 跳过 `key:Null`(yrs 表示已清除格式)。**测试**:`set_block_marks` 单测 + `frb_editor_binding_test.dart`(真实 `EditorController` 跑打字/加粗/分裂/改 heading/合并 → 重开 store 校验一致,Windows 3 项全绿)。**剩(🟡)**:应用内 localOffline UI 接线(当前为禁用占位)——把本地工作区/页面路由到这个 backend(单档 vs 本地页树,待定 UX)。 | 编辑引擎闭环 ✅;应用内 UI 待接 |
