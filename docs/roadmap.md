...
1. **可观测性:~~指标~~ ✅ ~~采集~~ ✅,**缺的是告警**(残留 M,`[需后端]`)—— 2026-08-04
   ...
   **但生产仍然没有任何自动化告警** —— 有了曲线不等于有人会被叫醒,而 08-03 那次正是
   "没人被叫醒"。所以它还排第一。
   下一步刻意**先看一周真实数据再定阈值**,而不是先设计告警体系再倒推指标 —— 阈值拍脑袋
   定出来的告警,要么天天响到没人看,要么永远不响。
...
2. **发布可信度:Windows Authenticode 签名**(M,**卡在买证书**)—— 更新器已校验 size+sha256,
   ...
   「不可信」。需要你买代码签名证书,我做不了。
3. **成熟度大件(用户挑一个)** —— 长文档虚拟化(L)/ 反链面板剩余(本地世界反链 + 关系图)/
   `<details>` 折叠第二步(规范形态要范围折叠,M–L)/ 并发编辑的 **marks 那半**(M–L,phase2
   §12 真正说的风险区)。
   **两个从这里划掉的**:① **同块字符级协同** —— 文本那半 2026-08-04 做完了,剩下的就是上面
   那个 marks;② **「表格补齐」是个空选项** —— 表格条目自己写着富行内单元格与矩形/行列选区
