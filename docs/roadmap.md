...
1. **待拍板(不写码)** —— M8:读侧自研 parser vs 采用 comrak。
   **2026-08-04 补:它其实已经被事实决定了,只是没人写下来。** 决定性的一条是双表示纪律
   (CLAUDE.md 原则 2):Markdown 语法逻辑**两端必须同步**,Rust 权威、Dart 镜像。comrak 只解决
   Rust 那半 —— 编辑器热路径在 Dart 里,用不上它。换过去之后两端会从「同一套手写逻辑」变成
   「comrak vs 手写」,而这正是最容易**静默分歧**的形态。
   佐证就在 0.13.13:那一版改了两处解析器,其中 `1~2` 不再是删除线是**刻意偏离 GitHub**
   (拿 GitHub 自己的渲染 API 实测过)。自研时是改几行;用 comrak 得 fork 或后处理。
   而放弃的收益不大 —— 记分牌已经 CommonMark 641/641、GFM 24/24。
   **仍留在这里的唯一理由**:这是个「决定」而不是「活」,该由用户拍一次板,拍完整条搬走。
2. **基建欠账** —— provisioning 层(L,方案在 `docs/cd-plan.md`,**刻意不含实施**:手工走一遍
