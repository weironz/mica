...
**未关闭条目(2026-08-27 逐条对着正文数,共 13 条)**:
...
| 5 | 导入完成发邮件 | M | — |
| ~~6~~ | ~~批量端点端到端测试(trash / restore / move 三个)~~ | — | ✅ 2026-08-27 关闭,7 条真库测试 |
| 7 | 侧栏多选:右键批量菜单 / 多行拖拽在 web 上未验 | S | **卡在工具**(合成事件送不进修饰键) |
| 8 | `Esc` 清空多选要不要上全局键盘处理器 | S | **未拍板** |
| 9 | MCP 删除面三处 | S | ①② 其实是"MCP 要不要覆盖协作面",**未拍板** |
| 10 | 多标签页剩余(快捷键 / 窄壳退化 / 是否持久化) | M | 持久化**未拍板** |
| 11 | 跟随画布浮层的 tooltip | M | **未拍板**(3.47 已把路径确定,代价没变) |
| 12 | 搜索 `ILIKE` 全表扫 | M | **未拍板**(难点是 CJK 分词) |
| 13 | yrs fuzz | — | 等上游合 PR |

**2026-08-27 当天动了 6 和 7,结果不对称,值得记**:

- **6 关掉了**(7 条真库测试)。这一条本来就该是"照搬已有写法",实际也是 —— 一小时内完成,
  中途只有两个意外,都在上面那条里写清楚了。
- **7 没关掉,而且卡的地方和预想的不一样**。原以为是"抽空跑一次 `just app chrome`",
  实际是 **Playwright 送不进修饰键 + 点击**,我为此白烧五次 web 构建去追一个不存在的 bug
  (`lessons.md`「自动化测试也会造假阴性」)。Ctrl/Shift 最终由**用户手动**确认可用;
  右键批量菜单和多行拖拽仍未验,因为它们都要先有一个多选。
  **教训对清单本身的意义**:`(S)` 标的是"改动小",不是"验起来容易"。这一条的成本全在验证侧,
  而清单里没有一栏表达这件事。

**接下来剩下的,全部要么等用户拍板、要么等上游**:8/9/10/11/12 五条是"做不做"而不是"怎么做";
13 等上游合 PR;1/2 是只有用户能定节奏的运维项;3/4/5 是有明确做法但没人提需求的存量项。

**顺带**:`main` 上有 **13 个** `v0.13.29` 之后的未发版提交(彩排文案、跨工作区标签页、
reorder 性能、删 Dart `moveView`、工具链钉死、侧栏多选 + 原子批量搬运)。发不发版是用户决定,
不进这张表。
〔这个数我第一次写成了 6 —— 数的是本次会话的提交,忘了 tag 之后还压着别的。用
`git log --oneline $(git describe --tags --abbrev=0)..main` 数,别凭印象。〕
