...
| 5 | 导入完成发邮件 | M | — |
| 6 | 批量端点端到端测试(trash / restore / move 三个) | S | — |
| 7 | 侧栏多选 web 端未验 | S | — |
...
**要我挑,下一步是 6 和 7 —— 都小、都关掉一个"现在不知道对不对"的面**:

- **7**(web 端点一遍多选)最该先做,因为它验的是**刚发出去的东西**,而且只要跑一次
  `just app chrome`。`lessons.md#4` 那条教训就是从这里来的。
- **6** 把 `batch-purge` 已有的那套真库测试照搬到另外三个批量端点上 —— 它们至今只有
  「SQL 跑得通」级别的保证,而 `batch-transfer` 这次已经证明这套写法能当场逮到东西。

其余要么**等用户拍板**(8/9/10/11/12,五条都是"做不做"而不是"怎么做"),要么等上游(13),
要么是只有用户能定节奏的运维项(1/2)。
