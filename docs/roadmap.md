...
- 🆕 **换头像后不同步到其他设备**(2026-08-07,v0.13.16 上线冒烟测发现)—— 在桌面端换头像,
  **本机看着换成功了**,但**远端设备上还是旧头像**。
  **⚠️ 关键观察(用户 2026-08-07 补测):对端重新登录后头像就正确了。** 这把范围缩掉一大半 ——
  **服务端存对了,持久化没问题**;问题在对端客户端**缓存了旧头像且不知道要重取**。
  代码里本来就有 `avatar_version`(换头像的响应里会返回新值),所以最可能的缺口是:
  它**没有传播到其他会话**,或者对端拿到了却没用它做缓存失效。
  **不是"正常现象"**:头像 URL 一旦被缓存,没有版本号参与就永远不会更新 —— 只是它的失败
  很温和(重登即好),所以容易被当成正常。
  **建议做法**:让 `avatar_version` 参与对端的缓存键(URL 带版本、或收到用户信息更新时失效),
  而不是去建实时推送 —— 头像不需要秒级同步,需要的是"下次拉用户信息时能发现它变了"。(S)
...
> **08-05 曾清空**(拍板不做的整条删除),**08-07 又进两条** —— 都是 v0.13.16 上线冒烟测
> 实测发现的,不是盘点想出来的。
