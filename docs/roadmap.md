...
- 🟡 **实时字符级并发协同:文本与 marks 都已落地,web 那半待 e2e 验证**(2026-08-04)—— 传输/应用层一直是**真 yrs merge**,不是 last-write。~~两端并发编辑同块合并成重复/拼接~~ ✅ **已修**。
...
3. **成熟度大件(用户挑一个)** —— 反链面板剩余(本地世界反链 + 关系图)/ `<details>` 折叠
   第二步(规范形态要范围折叠,M–L)。
   **三个从这里划掉的**:① **同块字符级协同已整条做完** —— 文本那半 2026-08-04 上午,
   **marks 那半同日下午**(`marks_diff_format_ops`,见 §可靠性与同步);② **「长文档虚拟化」也是个空选项**(2026-08-04 核实)—— 性能线 2026-07-23 就
