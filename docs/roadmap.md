...
- 🟡 **反向链接/引用面板/关系图** —— 正向 `[[` 已建;**引用面板已做**(云端页显示「谁链到我」可点跳转,7de2c2a)。**残留**:~~①并发扫描~~ ✅(buffered(8),6612330);~~②反向索引表~~ ✅(2026-08-05,1775b31);③本地世界(offline)反链;④**关系图**(graph view)。(各 S–L)`[需后端]`
  **②的经过**,留作方法记录:条目原写「规模成瓶颈再上,现在故意不建」——**没有任何数字支撑**。
  把生产快照还原到本地(798 篇 / 41 MB state),用 `#[ignore]` 的 `backlink_scan_cost`
  **调真的 `scan_backlinks`** 量:**release 690 ms**,而同库全文搜索只要 53 ms。贵 13×,
  且**跟着每次翻页触发**(搜索是用户主动敲的),所以这笔开销一直是隐形的。于是翻案照做:
  `document_yrs_base.link_targets uuid[]` + GIN(migration 0019),三条写路径**同语句 co-write**,
  面板退化为一条 `@>` 查询。**690 ms → 2–10 ms**。
  **顺手量出来的两件事**,都不在原计划里:① 启动回填一行一个 UPDATE,3784 行要 **43.8 s** ——
  它跑在服务接受流量之前,等于部署首启多 44 秒不可用;改成每批一个事务后 **6.7 s**。
  ② 正确性不能只看「反链能显示出来」:拿真实工作区**736 个目标逐一比对**索引与全量解码,
  384 个有反链、全部一致;并补了 both-directions 回归测试 —— **只会加不会减的索引**照样能通过
  所有「反链显示出来了」的测试,却让面板一直列着已被删掉的链接。
...
- ⏸️ **成熟度大件「挑一个」**(原第 3 项)—— 剩下两个候选**都不是缺陷,是功能广度**:
  反链剩余(云端引用面板**已能用**,反向索引表 2026-08-05 已做完 —— 690 ms → 2–10 ms;
  剩本地世界对等 + 关系图 —— 后者是产品下注)、
