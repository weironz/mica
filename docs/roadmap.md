...
- 🆕 **Traefik:证书过期重新变成无人看守 + 配置仍不在仓库**(2026-08-04 更新)——
  ...
  重建时它属于 blackbox 探测那一档,不是 `/metrics` 那一档。
...
1. **可观测性:~~指标~~ ✅、~~采集~~ ✅(给示例栈),告警 ⏸️ 不做,**缺的是机外黑盒探测**(M,`[需后端]`)—— 2026-08-04
   ...
   **还排第一的理由变了**:不再是"没有告警",而是**这一档的盲区在机器之外**。08-03 那次
   ...
   (08-03 那次就是),平均发现时间取决于你什么时候打开它。哪天有了第二个用户、或者
   开始对外承诺可用性,这条就该重新拿出来。
   **两个 `/metrics` 覆盖不到、别忘了的**:① **证书过期**(随 `uptime.yml` 一起没的);
   ② **「机器整个不行了」** —— 08-03 那次是整机 IO 饥饿,同机采集器一样会被饿死,
   所以这一档必须跑在这台机器**之外**。这两样合起来就是 blackbox 探测那一档。
2. **发布可信度:Windows Authenticode 签名**(M,**卡在买证书**)—— 更新器已校验 size+sha256,
   但用户装的仍是一个 SmartScreen 会告警的包。这是新用户看到的**第一样**东西,而它现在说
   「不可信」。需要你买代码签名证书,我做不了。
3. **成熟度大件(用户挑一个)** —— 反链面板剩余(本地世界反链 + 关系图)/ `<details>` 折叠
   第二步(规范形态要范围折叠,M–L)/ 并发编辑的 **marks 那半**(M–L,phase2 §12 真正说的
   风险区)。
   **三个从这里划掉的**:① **同块字符级协同** —— 文本那半 2026-08-04 做完了,剩下的就是上面
   那个 marks;② **「长文档虚拟化」也是个空选项**(2026-08-04 核实)—— 性能线 2026-07-23 就
   闭环了(三刀:painter 缓存 / 代码高亮记忆化 / 整块 layout 缓存),之后 `f4c8293` 又加了
   **paint 视口裁剪**(只录可见带)。真剩下的「屏外跳过**排版**」在归档条目里就写明是
   **有意不做**:两条架构约束(`performLayout` 拿不到滚动偏移、编辑器不自管视口)使它成为
   独立架构项,ROI 仅万级块/超长档 —— 而 paint 裁剪落地后每帧的贵活已被裁掉,ROI 只会更低;
   ③ **「表格补齐」是个空选项** —— 表格条目自己写着富行内单元格与矩形/行列选区
   **本来就能用**,而合并单元格、块级单元格、列宽都是**调研后有意不做**(与 markdown 权威 +
   round-trip 架构级互斥)。它在这份清单里挂了两版,读起来像还有活可干。
4. **待拍板(不写码)** —— M8:读侧自研 parser vs 采用 comrak。这是清单里唯一长期没拍的。
5. **基建欠账** —— provisioning 层(L,方案在 `docs/cd-plan.md`,**刻意不含实施**:手工走一遍
   还没人做过,没走过就写 IaC 等于把猜测固化)、每次 push 全档重编码的写放大(M,**动手前先读
   那条**:它解释了为什么显而易见的修法会造出它想避免的增长)。
   〔`/metrics` 不在这里 —— 第 1 项已经接管它,免得同一件事在两处各排一次。〕
