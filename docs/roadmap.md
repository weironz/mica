...
- 🟡 **实时字符级并发协同:文本已落地,marks 仍粗粒度**(2026-08-04)—— 传输/应用层一直是**真 yrs merge**,不是 last-write。~~两端并发编辑同块合并成重复/拼接~~ ✅ **已修**。
  ...
  **残留(这才是 phase2 §12 说的最大风险区)**:**marks 仍然粗粒度**——`marks_to_format_ops` 只加不减,所以「取消加粗」只能靠"全清+重放"表达。已加守卫:只有实际 marks 与目标不一致时才重写,普通打字只发文本拼接。并发**格式**操作仍可能互相覆盖(但不比改动前差:改动前每次击键连整段文本一起删)。真正的解 = delta↔marks 映射。
...
- 🆕 **拨测「每 15 分钟」是假的:一次真实故障它一次都没跑**(2026-08-04 实测)——
  `uptime.yml` 写的是 `cron: */15`,探 `/api/ready`(超时 20s),失败即 workflow 报错发邮件。
  **实际运行间隔**(08-02T17:14Z → 08-03T14:23Z,21 小时):12 次,而非应有的 84 次(**14%**);
  间隔先是 ~60 分钟,之后退化为 95 / 214 / 214 / 212 / 149 分钟。而**故障窗口是 16:19–16:45Z,
  上一次运行停在 14:23Z —— 整个故障期间一次都没跑**,是用户自己发现服务挂了。
  根因不是配置写错:**GitHub Actions 的 schedule 是 best-effort**,短间隔被丢得最狠。
  所以「有监控」这句话本身就是维护规矩第 2 条说的、**静默变假的否定式断言**。
  **修它要先拍板**:探针跑在被探对象之外才有意义,而 GH Actions 不可靠 → ① 换外部拨测服务
  (第三方依赖,按 CLAUDE.md 要走豁免);② 或节点上加 systemd timer 自探(**今天这种「机器活着
  但应用被 IO 饿死」它抓得到**,机器整个死了抓不到)。两者互补,但都不是纯技术选择。(S,待拍板)
  〔当天故障复盘:同机跑 `docker build`(neostor 的 vite + cargo,~1.4G)把 3.5G 无 swap 的机器
  压到 load 23 / IO 压力 98%,所有健康探针卡在 D 状态 → api/pg 标 unhealthy。Mica 进程没死。
  已加 4G swap(`vm.swappiness=10`,写进 fstab)作缓冲;**真正的解是把构建挪走**。〕
- 🟡 **Traefik:证书监控已做,配置仍不在仓库**(2026-07-29 核实)—— ~~过期无监控~~ ✅:`uptime.yml` 对两域名(app + s3)openssl 查证书剩余有效期,< 10 天(CERT_MIN_DAYS)即 fail → Actions 失败邮件。**注意它和拨测共用同一个 schedule**,所以上一条的「实际每 1–3.5 小时、可能整段不跑」对证书检查同样成立(证书是天级的,影响远小于拨测)。**残留**:Traefik 配置本体在仓库外未纳管;ACME 卡死那类故障仍靠 `deploy.md:86` 的手动 runbook。(S,external)
...
> **安全整档已关**(WS token TTL 心跳、token 出 query string、桌面 DPAPI、web 凭据走
> HttpOnly cookie、弱口令),整条搬进存档。08-03→08-04 又关掉五件:评论 Phase 1、
> `<details>` 折叠第一步、web outbox 背压、本地文档增量持久化、同块并发编辑的**文本**那半。
>
...
1. **监控是假的 —— 生产挂了没人告诉你**(S,**先拍板**)—— 2026-08-04 实测:`uptime.yml`
   声称每 15 分钟拨测,实际 21 小时只跑 12 次,**整个故障窗口一次都没跑**,是用户自己发现的。
   排第一不是因为它难,是因为**其余每一条都默认「出事你会知道」**,而这个前提现在是假的。
   见「生产运维与备份」小节;要外部拨测服务还是节点自探,是你的决定。
2. **发布可信度:Windows Authenticode 签名**(M,**卡在买证书**)—— 更新器已校验 size+sha256,
   但用户装的仍是一个 SmartScreen 会告警的包。这是新用户看到的**第一样**东西,而它现在说
   「不可信」。需要你买代码签名证书,我做不了。
3. **成熟度大件(用户挑一个)** —— 长文档虚拟化(L)/ 表格补齐 / 反链面板剩余(本地世界反链 +
   关系图)/ `<details>` 折叠第二步(规范形态要范围折叠,M–L)。**同块字符级协同已不在此列**:
   文本那半 2026-08-04 做完了,剩下的 marks 那半见 §可靠性与同步(那才是 phase2 §12 说的
   风险区)。
4. **待拍板(不写码)** —— M8:读侧自研 parser vs 采用 comrak。这是清单里唯一长期没拍的。
5. **基建欠账** —— provisioning 层(L,方案在 `docs/cd-plan.md`,**刻意不含实施**:手工走一遍
   还没人做过,没走过就写 IaC 等于把猜测固化)、无 `/metrics`(M)、每次 push 全档重编码的
   写放大(M,**动手前先读那条**:它解释了为什么显而易见的修法会造出它想避免的增长)。

**下一轮核实的重点**:**未核实的 2 条**(fuzz 覆盖面、隐私声明/ToS),以及所有 🟡 条目的
「剩下那半」—— 部分完成的条目最容易整条烂掉:它已经有了一个 ✅,看上去就像做完了。
另外**排序清单本身要跟着条目走**:这一轮就是它落后于条目结论才留下一句假话。
