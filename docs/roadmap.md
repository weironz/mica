...
- ⏸️ **可上传携带脚本的 SVG —— 纵深项,拍板不做架构改动**(2026-08-05 复核)—— 允许 `image/svg+xml`,blob 端点 `Redirect::temporary` **跳存储的 `download_url`**,SVG 脚本因此跑在**存储源**、碰不到 app 的 token —— 不是账号接管 XSS。**仅当**运营者把 `S3_PUBLIC_BASE_URL` 配成与 app 同源才成洞。
  **2026-08-05 实测生产是安全形态**:`S3_PUBLIC_BASE_URL` **未设置** → 走 presigned GET,落在存储主机上,与 app 跨源。
  **本轮做的不是改架构,是让误配会说话**:启动时若 `S3_PUBLIC_BASE_URL` 与 `MICA_APP_BASE_URL` 同源即 `warn!`,和旁边那条「非环回即警告明文 HTTP」同一个模式。**告警而非拒启动**:app 无法断定同源就是错的(反代可能把 /blobs 路由到别处),为一个运营者可能有意为之的配置拒绝启动,比说出来更糟。
  **架构层面的加固(强制 attachment / 上传即拒 SVG)不做**:302-跳存储的架构下 app 不发字节,要么改成 app 代理字节(丢掉这个架构的全部好处),要么给 presigned 加 `response-content-disposition`(只对 presigned 路径有效,对 CDN 路径无效)。为一个**当前配置下不存在**的洞付这个代价不划算。(`files.rs`, `main.rs`)

- ⏸️ **自托管 TLS 全靠运维 —— 硬闸门拍板不做,但原因不是"做不到"**(2026-08-05 更正)—— ~~无启动告警~~ ✅ 早在 `9289ecf` 就有:`main.rs` 绑定后判 `!addr.ip().is_loopback()`,非环回即 warn「本进程只说明文 HTTP,请在前面终止 TLS,否则连 WS URL 里那个 token 都在网上裸奔」。默认 `127.0.0.1` 安全且**静默** —— 只有真承担了风险的部署才看到告警。
  **⚠️ 原条目的理由是错的,已更正**:它写「要拒得先能看见前面有没有反代,单机上看不见」,于是被当成做不到。**探测不到不等于做不到** —— 标准做法是不探测而让运营者**声明**(Rails `force_ssl`、Django `SECURE_PROXY_SSL_HEADER` 都是这个套路):生产模式下绑非环回必须显式给一个信任反代的开关,否则拒启动。所以它是**能做的**(S–M)。
  **拍板不做的真实理由**:① 它要新增一个配置旋钮,而仓库里**没有**现成的可复用 —— 限流那处是**刻意**"走 XFF 从右往左取第一个公网地址,而不是信任拓扑";② 本部署在 Traefik 后面、配错时告警已经会响;③ 受益的是未来的自托管者,不是今天。**哪天对外分发自托管镜像,这条要重新拿出来** —— 那时"配错就明文裸奔"会变成别人的问题,而告警只是告警。(S–M) `[需后端]`
...
- 🟡 **反向链接/引用面板/关系图** —— 正向 `[[` 已建;**引用面板已做**(云端页显示「谁链到我」可点跳转,`GET .../backlinks` 按需扫描、复用 page_link_targets,7de2c2a)。**残留**:~~①并发扫描~~ ✅(buffered(8),6612330);②**反向索引表 —— 2026-08-05 实测,原来那句「规模成瓶颈再上,现在故意不建」已经到点了**;③本地世界(offline)反链;④**关系图**(graph view)。(各 S–L)`[需后端]`
  **②的实测**:把生产快照还原到本地(798 篇文档 / 41 MB state),用 `#[ignore]` 的
  `backlink_scan_cost` **调真的 `scan_backlinks`**(不是重写一遍)——**release 稳态 ~690 ms**
  (首次 1.04 s 冷缓存;debug 是 1.4–1.7 s)。三个不同目标耗时几乎相同、命中都是 0,
  正说明**成本与结果无关**:面板必须完整所以**没有提前退出**,每开一次就是
  **798 次 DB 往返 + 798 次全量 CRDT 解码**,只被 `buffered(8)` 限并发。
  **对照**:同库上全文搜索是 53 ms —— 反链贵 13×,而且它**跟着每次翻页触发**,搜索是用户主动敲的。
  **所以这条从「故意不建」改成「该建了」**:形状是现成的,就是**全文搜索 M1 那一招**再用一次 ——
  给 `document_yrs_base` 加一列/一张 link 表,与 state **同语句 co-write**(红线#1),
  把 N 次解码换成一条索引查询。注意 `content_text` **接不上**:链接目标在块的 `data` 里,
  不在正文投影里。(M) `[需后端]`
...
> **这是 top-N,不是清单全文。** 上面正文有 **33 条**(2026-08-05 复核),其中 3 条是 ⏸️ 已拍板
> 不做,**其余 30 条都有真实的未做部分**。这一节只排「下一步最该动手的」,所以它短不等于活干完了 ——
...
> —— §安全 小节仍有 **3 条**(SVG 纵深、TLS 硬闸门、清单卫生),只是都不到底线级。
...
- ⏸️ **成熟度大件「挑一个」**(原第 3 项)—— 剩下两个候选**都不是缺陷,是功能广度**:
  反链剩余(云端引用面板**已能用**;2026-08-05 实测按需扫描 798 篇要 ~690 ms、贵过全文搜索 13×,
  **反向索引表已从「故意不建」翻成「该建了」**;另剩本地世界对等 + 关系图 —— 后者是产品下注)、
