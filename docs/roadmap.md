...
- 🟡 **页面属性/标签**(M1 进行中,2026-07-22)—— 走 front matter 权威路(调研定论:同类 md 权威系均如此,见 `docs/page-properties.md`)。**已落地**:数据/权威层——Rust `crates/markdown/src/properties.rs`(解析扁平子集 + 类型推断 + 外科式写回,round-trip 不变量经用户批准从字节保真降为规范化子集稳定)+ Dart 镜像 `properties.dart`,两端逐条测试一致(Rust 9 / Dart 10 全绿)。tags = `tags:` list 属性(值 M2 喂现有 ref 索引)。**残留(M1 UI)**:页头属性面板(读 root 块 `data['front_matter']` → 解析显示/tags chips → 编辑经 `setBlockDataJson(rootBlockId)` 写回 CRDT),接线要穿 controller op → CloudSyncSession → FFI/Yjs 多层且跨平台(桌面 FFI vs web),贴同步边界(红线#1),留干净上下文起步。**M2**:tag 可点可搜 + 属性筛选/排序。**数据库视图(带类型列/筛选/relation)另立项**——与 markdown 权威+round-trip 架构互斥,要么破双表示红线要么豁免 md 权威(AFFiNE/siyuan 路),是独立大决策。(L) `[需后端]`
