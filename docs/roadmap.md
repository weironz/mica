...
> **这一节又空了(2026-08-06)。** 不是漏删:同块字符级协同(文本 + marks)、长离线重连
> (背压 + 合并)两条都已关闭 —— 后者 08-05 撤回过一次,08-06 连同它撤回的原因一起做完了
> (下方条目留到发版时搬进 `roadmap-done.md`)。留着标题是因为这是最该有人盯的一档 ——
> 空着本身是个状态,而不是这一档不存在。

- 🟡 **长离线重连:合并已回滚,ack 协议挡着**(2026-08-05 做了又撤)——
  ~~① web 侧无界~~ ✅(2026-08-03,内存 outbox 走同一个 `_pushWindow = 64`);
  **② 合并:撤回了**。
  **量是真的**(`offline_reconnect_merge_measurement`,`#[ignore]`,还留着):攒 N 条后重连,
  10 条 129→11ms、50 条 523→10ms、**200 条 2702→19ms**;乘数是**攒了多少条**、没有上界,
  与写放大(乘数是文档大小、有上界)不是一回事。
  **但实现是错的,CI 抓到,已整条回退到 1feef919 的状态**。原条目写着「协议不用动 ——
  合并后用最高 clock 当 id,ack 让 `pushed_clock` 推进到那里,整段作为前缀一次裁掉」——
  **这句没验证过,而且是假的**。ack 路径是
  `while (_ackedAhead.remove(pushed + 1)) pushed++` ——`pushed_clock` 只沿**逐条被 ack**的
  连续前缀推进。一条合并推送以 id 50 被 ack 时,clock 1–49 从没单独 ack 过,前缀断在 1,
  **outbox 永远排不空**。
  **代价还不止**:合并让服务端回显的 id 无法再指认某一条编辑,「拒绝 clock 1」这件事
  表达不出来 —— 两条 P2b 回归(「rejected push 必被重试、绝不丢」)因此变红,而
  `just test` 碰不到它们(integration_test 要 Windows 设备、只在 CI 跑),红了六次提交没人看。
  ~~**真要做,前置是改 ack 语义**:让 id N 表示「N 及以前全部落库」,那是协议改动,不是优化。~~
  ✅ **2026-08-06 做完了,而且不用改协议 —— 上面那句判重了**。服务端把 `id` 当**不透明回显**
  (`ws.rs` 取 `envelope.id` 原样回传),所以「id N = N 及以前全部落库」这个语义**完全活在客户端**,
  `WS_PROTOCOL_VERSION` 一动没动。缺的从来不是协议,是**客户端不知道每次推送覆盖了哪段** ——
  补上 `_runFrom`(记 `(from..through]`)后,ack 按**区间**走连续前缀而不是逐条 `pushed+1`,
  整段一次裁掉。
  **上次丢掉的那半也补回来了**:合并确实让服务端说不出「是哪条编辑有问题」,做法是
  `_deMergeFrom` —— 合并推送被拒时**回退到区间起点、逐条重发**,毒药条目被隔离,它周围的照样
  过得去。**粒度只在顺利时花掉,一出问题立刻买回来**。合并另外只在真积压时才做
  (`_mergeMinRun = 8`),这是 30c6da6「无条件合并打断逐条重试」的教训。
  **闸门这次是有的**:两条新回归进 `cloud_sync_integrity_test.dart`(本机 Windows 跑得起来,
  上次以为它只在 CI 跑,所以红了六次没人看)。**做了 mutation 验证**:把 ack 推进换回旧的逐条
  走法,「重连积压一次推完」当场变红、并且是 `drainOutbox` 超时 —— 正是当初那个症状。
  复测数字比原记录还好:**200 条 2885→66ms(43.6x)**,字节 3426→908。(已完成)
