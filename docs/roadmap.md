...
> 2026-07-22 新增小节。当时的问题是**多处**「删除不真删」+「无界追加」,单节点小盘上会
> 慢慢暴雷 —— 这类问题没有告警会响,只有某天写不进去。
>
> **2026-08-06 复核:「多处」已经不成立** —— 这一档建档后关掉 13 条,只剩下面 1 条,而且
> 那条不是缺陷、是"还没有上限"。开场白留着旧数量正是规矩第 2 条说的那种静默变假:小节
> 描述写的是建档当天的状态,而条目一条条搬走时**没有任何东西会去改它**。
...
- 🟡 **容量配额:blob 已封,其余仍无**(2026-07-30)—— ~~无任何容量配额~~ 部分已做:
  ...
  **仍然没有的**:单文档大小上限、用户级/实例级总量上限;WS 单条消息**没有显式上限**
  ——全仓没有一处设过 `max_message_size`/`max_frame_size`,走的是 axum/tungstenite 的默认值
  (⚠️ 2026-08-06 更正:上一版这里写「`ws.rs:60` 默认仍可收 64MiB」,**那个行号早已漂走**
  ——今天的 `ws.rs:60` 是 `struct UpdatePayload`,与消息上限无关。按图索骥会扑空:
  行号引用会烂,符号名不会);大文档写放大照旧(每 push 全量 base 覆写 + 每 10min 全量版本,
  该条 2026-08-05 已量化后拍板暂不做)。**未实测的一处**:import-url 的配额检查没能端到端
  验到 —— SSRF 守卫先拒了 loopback 测试 URL(顺序正确),代码路径与另三条一致但只经代码核实。

  **2026-08-06 扒了三个参照系,结论是这条基本可以关掉**:

  | | 单文件上限 | 总量上限 | 单文档大小上限 |
  | --- | --- | --- | --- |
  | **Notion** | 5 MB 免费 / 5 GB 付费 | **明确没有**(官方:账号与工作区均无总量限制) | 无(只有块数,且 2022 起单人工作区已取消) |
  | **AFFiNE** | 有(`BlobQuotaExceeded`) | 有(`StorageQuotaExceeded`) | 未见 |
  | **AppFlowy** | 7 MB(免费云) | 5 GB(免费云);自托管不限 | 未见 |
  | **Mica 今天** | 25 MiB(`S3_MAX_UPLOAD_BYTES`) | 1 GiB / workspace | 无 |

  两条判断:
  ① **「单文档大小上限」三家一个都没做** —— 文档是渐进长大的,硬上限的失败模式是
  「用户正编辑到一半被拒写」,而那时候东西已经在他屏幕上了。真正的控制点是块数或存储总量,
  不是单文档字节数。**建议整条划掉,不做**。
  ② **总量配额在别家是计费机制,不是防盘满机制** —— AFFiNE/AppFlowy 有是因为要分层收费,
  Notion 不收这个钱所以干脆不做。Mica 单人自托管没有计费诉求,它的 1 GiB 起的确实是防盘满
  作用 —— **不同动机、同一手段,而这个手段已经有了**。所以「用户级/实例级总量上限」也
  缺乏真实驱动:单人实例上,per-workspace 上限已经是那个天花板。
  (`files.rs` `ensure_storable`, `store.rs` `workspace_bytes_used`, `sync.rs`)(M) `[需后端]`
