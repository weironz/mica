...
> **第二段:导入本身是用过、能用的功能**(操作者手工导过归档),没被整条串起来跑过的是
> 「取出 → 打包 → 导入 → 比对」这条链,而按 2026-08-30 的定位它**不需要**被当成恢复
> 路径来验。要知道的形态差是:导入端点吃的是 **zip 不是目录**
> (`read_zip`,单次上限 256 MiB),而且它是**通用 Markdown 导入器,不是 round-trip 还原**:
> 页面 id、CRDT 历史、评论、分享链接、账号与成员关系**都不会回来**(那些只在数据库里),
> 导进去的是**文字相同的新页面**。
>
> **定位(用户 2026-08-30 拍板)**:**备份恢复以数据库 + 对象为准**(已端到端验证),
> `content` 是**兜底** —— 以及「笔记不依赖 Mica 也能读」。所以它不进演练,也不该被
> 列成一条待验证的恢复腿。细节见 [`backup.md`](backup.md)。
...
### 6.1 ⏸️ ~~可用性的正确下一档:warm standby~~(**已拍板不做**,2026-08-30)
...
**2026-08-30 拍板不做**(连同「双实例,一个做冷备」这个提法一起)。当时摆在桌上的是
两档:①零代码的常驻冷备(把 DR workflow 的目标从一次性机器改成常驻机器,定期跑,
得到逐字节等同的实例);②本节这条流复制。两档都不做。

**记下一条这一节原本没写、而它是选择放弃的一部分**:做流复制**必须让 postgres 端口
对另一台机器可达**,而「`POSTGRES_PASSWORD` 用默认值是安全的」这个论证的**全部依据**
就是两份 compose 都不发布该端口。改拓扑就得同时补上真口令 + 私网或 TLS ——
**一个靠拓扑维持的安全性质,在改拓扑那一刻失效**,这和 S3 默认值是同一类。

重新提这条之前,先确认前提有没有变:RTO 现在是 **11 分 50 秒**(§3,CI 无人值守实测),
而这条买的是把它压到「几分钟」。当初写它时 RTO 还是「未实测」。
