...
## 8. 事故:异地数据库备份静默中断 29 天(2026-07-31 ~ 08-29)
...
- `2026-07-31 03:00` —— 最后一份成功的 `_pgdump` 快照
- `2026-08-15` —— CLI 退休 `MICA_TOKEN`(compose 未跟进)
- `2026-08-28` —— PG 升到 18.6(客户端未跟进);当晚我部署 v0.13.37,备份容器重建
- `2026-08-29` —— 发现。手工补一份 dump 进仓库;三处修复入库

⚠️ **07-31 到 08-15 之间那 15 天另有原因,查不出来了** —— 备份容器 08-28 被部署重建,
日志已随之丢失。只能确定「至少从 07-31 起没有新快照」,不能声称全程同一个原因。
**这本身是个教训:容器日志不是取证介质。**
