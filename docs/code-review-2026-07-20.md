...
### 已上生产：v0.12.10（2026-07-21）

本报告涉及的全部修复已随 **v0.12.10** 部署到生产并实测验证。冒烟判据用的是**同一份文档在部署前后的读回差异**（存储数据一字未动，只换渲染器）：

| | 0.12.9（部署前） | 0.12.10（部署后） |
| --- | --- | --- |
| `` [`useState`](https://react.dev/…) `` | `` `useState` `` —— URL 消失 | 完整保留 |
| `` [see `useEffect` docs](…) ``（code 严格在 link 内） | 正常 | 正常（无回归） |
| `` `useMemo` ``（纯 code） | 正常 | 正常（无回归） |

两个对照组是刻意设计的：它们证明修复不是「链接全都保住了」这种大水漫灌，而是**精确地**只改变了范围完全重合那一种。`/api/health` 报 `0.12.10`。
