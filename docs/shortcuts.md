...
| `Ctrl + Shift + F` | 全局搜索(默认跨全部工作区,当前工作区优先排序) |
| `Ctrl + 点击` / `Ctrl + ↵`(搜索结果上) | 开到**新标签页**而不是原地替换 —— 命中在别的工作区时,这是唯一能跟过去看又不把外壳切走的方式(标签页自己带工作区) |
| `Ctrl + ,` | 打开设置 |
| `F2` | 重命名侧栏高亮的那一行(页面/文件夹),原地改名 |
| `Ctrl + 左键`(侧栏树) | 把这一行加进多选 / 取出 |
| `Shift + 左键`(侧栏树) | 从锚点选到这里(同一父级、同一段连续可见行内) |

> `Ctrl + ,` 会被中文输入法在 OS 层吞掉(标点切换),此时切英文输入法,或从菜单进设置。

> **侧栏多选的 Shift 为什么只在同一父级内生效**:树里「从这儿选到那儿」一旦跨了父级就没有
> 确定含义——中间那些文件夹算不算?它们**折叠着**的子节点算不算?没有一个答案是显然的,而
> 猜错的代价是批量删除删掉了看不见的东西。同类产品全都在躲这件事:AppFlowy 侧栏**根本没有
> 多选**(开着 feature request);AFFiNE 只在扁平的 "All docs" 列表里用复选框做;Windows
> 资源管理器——也就是这个需求的参照物——只在**右侧扁平面板**里多选,左侧目录树里不行。
> 所以规则是绕开而不是回答:同父级、同一段连续可见行内 Shift 精确成段,跨父级时退化成单点
> 加选。范围里夹着一个**展开的**文件夹不算跨父级(它的子行只是被跳过,不会被选中——选中
> 文件夹本身已经等于带上整棵子树)。规则实现在 `ui/tree_selection.dart`,配 41 条测试。
>
> 三个批量入口共用同一条判定(`actsOnSelection`):右键、拖拽、以及右键菜单里的动作。
> 右键点在选区**外面**的行 = 只对那一行操作,和所有文件管理器一致。
>
> **「Esc 清空多选」走全局 `HardwareKeyboard` 处理器,不是任何 `Shortcuts` 绑定**
> (2026-08-28 用户要求后加上;此前「没有 Esc」是决定,因为三种焦点绑法都写过、都在真机上
> 实测、三种都是死的:树本地的 `Shortcuts`/`Actions`、给树加 `FocusNode` 后再绑、以及
> `_appShortcuts` —— `Ctrl+N` 就在那儿且能用)。探针给出的事实:选中操作后 400ms
> `primaryFocus` 确实是侧栏,但按下 Esc 时焦点已经回到**页面路由自己的 scope** —— 那是
> 整个 shell 的祖先,而 Flutter 从焦点节点**向上**分发,于是 shell 内部注册的任何快捷键都
> 看不到这个键。全局处理器不看焦点,这正是其它绑法缺的性质。
> 当时记的代价「对话框/行内改名正拿着 Esc 时会一起触发」用两条守卫解掉
> (`escClearsTreeSelection`,配测试):对话框在上(路由非 current)不清;行内改名中不清。
> 处理器**只观察、永不吞键**(与外点清空的 Listener 同一哲学),所以查找栏/斜杠菜单照常
> 收到自己的 Esc。**点侧栏外任意处也清空多选** —— 与清掉「定位」的是同一个 Listener,
> 对话框和右键菜单住在 Overlay 里、点它们不经过这个 Listener,所以批量菜单开着时选区不掉。
> 点任意一行、点树下方空白区这两条老路径不变。
