...
>
> **没有「Esc 清空多选」,这是决定不是遗漏。** 三种绑法都写过、都在真机上实测,三种都是死的:
> 树本地的 `Shortcuts`/`Actions`、给树加 `FocusNode` 后再绑、以及 `_appShortcuts`(`Ctrl+N`
> 就在那儿且能用)。探针给出的事实:选中操作后 400ms `primaryFocus` 确实是侧栏,但按下 Esc
> 时焦点已经回到**页面路由自己的 scope** —— 那是整个 shell 的祖先,而 Flutter 从焦点节点
> **向上**分发,于是 shell 内部注册的任何快捷键都看不到这个键。要做只能上全局
> `HardwareKeyboard` 处理器,而它会在对话框/行内改名正拿着 Esc 时一起触发。
> 清空多选走点任意一行、或点树下方空白区(两条都实测可用)。
